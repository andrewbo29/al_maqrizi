...
class RegionsFilter(ArgParseMixin):
    ...
    def add_args(self, parser):   
        parser.add_argument('--dbscan_eps', type=float, help="eps parameter for DBSCAN algorithm used to detect outlier regions", default=0.3)
        parser.add_argument('--dbscan_min_samples', type=int, help="min_samples parameter for DBSCAN algorithm used to detect outlier regions", default=10)
        parser.add_argument('--max_major_axis_length', type=int, help="filter regions with major_axis_length greater then this value", default=160)
        parser.add_argument('--min_major_axis_length', type=int, help="filter regions with major_axis_length lower then this value", default=15)
        parser.add_argument('--min_minor_axis_length', type=int, help="filter regions with minor_axis_length lower then this value", default=7)
        parser.add_argument('--max_major_minor_axis_length_ratio', type=float, help="filter regions with major_axis_length/minor_axis_length ration greater then this value", default=10.)
...
class MultiplyRegionResizer(ArgParseMixin, AbstractRegionResizer):
    ...
    def add_args(self, parser):
        parser.add_argument('--bbox_mul', type=float, help="enlarge output patches bounding box by given factor", default=1.2)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Extract patches from given file to given dir basing on otsu-binarization and connected componetnts detection')
    ...
    parser.add_argument('-ob', '--output_binary', help="write binarized images to output", default=False, action='store_true')
    parser.add_argument('-oiw', '--output_images_width', type=int, help="output images width", required=False, default=None)
    parser.add_argument('-oih', '--output_images_height', type=int, help="output images height", required=False, default=None)
