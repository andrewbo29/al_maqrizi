...
\title[]{\Large Верификация авторства\\ арабских рукописей при помощи\\ глубоких 
свёрточных сетей}
...
\begin{frame}
	...
	\begin{itemize}
		\item Верификация авторства аль-Макризи\footnote{Аль-Макризи (1364--1442) --- египетский историк и географ периода мамлюков.} недавно найденной части рукописи \textit{al-Khitat} (историко-географический обзор Египта).
	\end{itemize}		
...
\begin{frame}
	...
	\begin{itemize}
		\item Скользящее окно: разбиение изображения на квадраты $80\times 80$ с шагом в $20$ пикселей.
		\bigskip
		\item Связные компоненты:
			\begin{enumerate}
				\item Бинаризация изоборажение с помощью фильтра Otsu.
				\item Выделение компонент связности из бинаризованного изображения.
				\item Фильтрация компонент на основе их размеров
				\item Выделение компонент аутлаеров с помощью алгоритма DBSCAN.
				\item Извлечение AABB\footnote{Axis-aligned minimum bounding box} компонент связности и преобразование их в квадраты $28\times 28$ пикселей.
			\end{enumerate}
	\end{itemize}
