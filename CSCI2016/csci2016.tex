...
We consider author verification as a binary classification problem: positive class designated as 1 consist of images that belongs to al-Maqrizi's hand and negative class designated as 0 consist of images of different author. In this context our goal is to build a classification pipeline able to estimate the probability (\textit{soft} classification) that a given image belongs to the $1$ (al-Maqrizi) class. Thus, perfect classifier should return 1 for all al-Maqrizi's images and 0 for all non-al-Maqrizi's images. The entire proposed classification pipeline for al-Maqrizi authorship verification illustrated at Figure~\ref{fig:pipeline} and consists of the following steps:
...
Note: since this step is done only to unify our images data set we did not include it to the pipeline figure~\ref{fig:pipeline}.
