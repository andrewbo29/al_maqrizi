...
\begin{abstract}
In this paper consider the problem of an automatic authorship verification of manuscripts written in Arabic. This manuscripts are treated as an images. We propose the method based on accordingly trained deep convolutional neural network binary classifier which using as an inputs patches from image. We consider two types of patches extraction: sliding window and connected components. Obtained probabilities utilize for defining of the manuscript authorship. This method we use for author verification of the work of medieval Egyptian historian al-Maqrizi \textit{al-Khitat} which volume was recently discovered. For training deep network was used a number of labeled data from al-Maqrizi and non-al-Maqrizi manuscripts. Our method demonstrate promising results in this problem and thus is a useful tool for historians and philologists working in the area of authorship verification.    
\end{abstract}
